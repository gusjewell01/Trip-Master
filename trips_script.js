...
if('trip' in localStorage){
    trips.push(JSON.parse(localStorage.trip));

    document.getElementById("new_trip_body").style.display = "none";
...
if('trip' in localStorage){
    var map = new google.maps.Map(document.getElementById('other_map'), mapOptions);
} else {
...
function calcStop() {
    ...
    console.log(trips[0].getStopNames());
    var request = {
        origin: trips[0].getStopNames()[trips[0].stopNames.length -1],
        destination: trips[0].getStopNames()[trips[0].stopNames.length],
        travelMode: google.maps.TravelMode.DRIVING, //WALKING, BYCYCLING, TRANSIT
        unitSystem: google.maps.UnitSystem.IMPERIAL
    }
    ...
    directionsService.route(request, function (result, status) {
        if (status == google.maps.DirectionsStatus.OK) {
            ...
            const output = document.querySelector('#output');
            output.innerHTML = "<br><div class='alert-info'>From: " + trips[0].getStopNames()[trips[0].stopNames.length -1] + ".<br />To: " + trips[0].getStopNames()[stopCounter] +
...
if('trip' in localStorage){
    ...
    directionsService.route(request, function (result, status) {
        if (status == google.maps.DirectionsStatus.OK) {
            ...
            document.querySelector('#trip__Name').innerHTML=trips[0].name;
            const output = document.querySelector('#new_output');
            output.innerHTML = "<div class='alert-info'>From: " + trips[0].origin + ".<br />To: " + trips[0].destination + ".<br /> Driving distance <i class='fas fa-road'></i> : " + result.routes[0].legs[0].distance.text + ".<br />Duration <i class='fas fa-hourglass-start'></i> : " + result.routes[0].legs[0].duration.text + ".</div>";
            // trips[0].distances.push(result.routes[0].legs[0].distance.text);
            // trips[0].durations.push(result.routes[0].legs[0].duration.text);
            //display route
            directionsDisplay.setDirections(result);
...
function addStop() {
    ...
    trips[0].stopNames.push((document.getElementById("stop").value));
    var request = {
        origin: trips[0].stopNames[trips[0].stopNames.length -2],
        destination: trips[0].stopNames[trips[0].stopNames.length-1],
        travelMode: google.maps.TravelMode.DRIVING, //WALKING, BYCYCLING, TRANSIT
        unitSystem: google.maps.UnitSystem.IMPERIAL
    }
    ...
    directionsService.route(request, function (result, status) {
        if (status == google.maps.DirectionsStatus.OK) {
            ...
            const output = document.querySelector('#output');
            output.innerHTML = "<br><div class='alert-info'>From: " + trips[0].stopNames[trips[0].stopNames.length -2] + ".<br />To: " + trips[0].stopNames[trips[0].stopNames.length -2] +
                ".<br /> Driving distance <i class='fas fa-road'></i> : " + result.routes[0].legs[0].distance.text +
                ".<br />Duration <i class='fas fa-hourglass-start'></i> : " + result.routes[0].legs[0].duration.text + ".</div>";
            ...
            console.log(trips[0].distances.length);
            var newStop = new Stop(trips[0].stopNames.length -1 , trips[0].stopNames[trips[0].stopNames.length-1], trips[0].distances[trips[0].distances.length-1], trips[0].durations[trips[0].durations.length -1]);
            
            trips[0].stops.push(newStop);
            console.log(trips[0].stops[trips[0].stops.length -1]);
            
            let stopNumber = trips[0].stops[trips[0].stops.length -1].number;
            let stopName = trips[0].stops[trips[0].stops.length -1].name;
            let stopDistance = trips[0].distances[trips[0].distances.length -1];
            let stopDuration = trips[0].durations[trips[0].durations.length -1];
